...
{
    int ret = 0;
    if (_stream->available() <= 0 && _sidebuff_w == _sidebuff_r) { // nothing to do
        return ret;
    }
    do
    {
        ...
        else if (_sidebuff_w != _sidebuff_r) {
            c = _sidebuff[_sidebuff_r];
            _sidebuff_r += 1;
            _sidebuff_r %= SIDEBUFF_SIZE;
        }
        ret += 1;
        if (_echo) {
    ...
    while ((_stream->available() > 0 || _sidebuff_w != _sidebuff_r) && _higher_priori);
    return ret;
}
...
int SerialCmdLine::side_enter(char c)
{
    uint16_t nw = (_sidebuff_w + 1) % SIDEBUFF_SIZE;
    if (nw != _sidebuff_r)
    {
        _sidebuff[_sidebuff_w] = c;
        _sidebuff_w = nw;
        return task();
    }
    else
